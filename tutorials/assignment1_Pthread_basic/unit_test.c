...
int main(int argc, char **argv)
{
  ...
  for (nThreads = 4; nThreads <= 4; nThreads++) {
    ...
    for (i = 0; i < NUMGAMES; i++) {
      if (strcmp(results_ref[i].name1, results[i].name1)  ||
			    strcmp(results_ref[i].name2, results[i].name2)  ||
			    results_ref[i].goals1 != results[i].goals1 ||
			    results_ref[i].goals2 != results[i].goals2 
			    ) {
